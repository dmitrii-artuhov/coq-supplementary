...
Section S.
  ...
  *)
  ...
  Proof.
    intros. split.
    - intros. unfold update. apply st_binds_tl.
      + symmetry. assumption.
      + assumption.
    - intros. unfold update in H. inversion H. subst.
      + contradiction.
      + subst. assumption.
  Qed.  
  ...
  Proof.
    ...
    - intros.
      inversion H. subst.
        ...
        + subst. unfold update in *. apply st_binds_tl.
          * assumption.
          * apply st_binds_tl. assumption. assumption.
  Qed.
  ...
  Proof.
    destruct (id_eq_dec x1 x2).
      - subst. remember (state_deterministic st x2 n1 m). rewrite <- e. apply update_eq.
        + assumption.
        + assumption.
      - apply update_neq.
        + assumption.
  ...
  Lemma update_permute (st : state) (x1 x2 x3 : id) (n1 n2 m : A)
    ...
    st [x1 <- n2][x2 <- n1] / x3 => m.
  Proof.
    intros. unfold update in *.
    dependent destruction SM. subst. 
      - apply st_binds_tl.
        + symmetry. assumption.
        + apply st_binds_hd.
  ...
  Proof.
    unfold state_equivalence. intros. reflexivity.
  Qed.

  Lemma st_equiv_symm (st st': state) (H: st ~~ st') : st' ~~ st.
  Proof.
    unfold state_equivalence. intros. symmetry. apply H.
  Qed.

  Lemma st_equiv_trans (st st' st'': state) (H1: st ~~ st') (H2: st' ~~ st'') : st ~~ st''.
  Proof.
    unfold state_equivalence. intros. split.
